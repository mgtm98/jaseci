...
"""
        ...
        try:
            ...
            cmd = ["jac", "dev", str(test_file), "--port", str(port)]
            ...
            # Wait additional time for backend server to fully initialize
            # In dev mode, there are two servers (frontend and backend)
            time.sleep(5)

            # Test initial content (Content 1) with retries
            max_retries = 30
            retry_interval = 2
            success = False
            ...
            # Test updated content (Content 2) with retries
            success = False
            for attempt in range(max_retries):
                try:
                    response = requests.post(
